...
static void AccumulateBlend(CByteImage& img, CFloatImage& acc, CTransform3x3 M, float blendWidth)
{
    ...
    for (int y = bb_min_y; y <= bb_max_y; y++) {
        for (int x = bb_min_x; x < bb_max_x; x++) {
            ...
            // piecewise horizontal hat function
            CShape sh = img.Shape();
            float slope = 1 / blendWidth;
            // from 0 to blendWidth/2 - 1: accending
            if(x_src < (blendWidth/2 - 1)){
                weight = x_src * slope;
            }
            // from (blendWidth/2) to (sh.width - blendWidth/2 - 1): flat
            else if(x_src < sh.width - blendWidth/2 -1){
                weight = 1.0; // flat
            }
            // from (sh.width - blendWidth/2) to (sh.width - 1): descending
            else{
                weight = (sh.width - x_src - 1) * slope;
            }
			// // *** END TODO ***	
...
CByteImage BlendImages(CImagePositionV& ipv, float blendWidth)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        CVector3 corners[4]; // represent 4 coorners' coordinates
    ...
    if (debug_comp)
        ...
        // crop and trim
        // if pano is false, do not crop and trim, otherwise, crop and trim
        bool pano = false;
        int finalWidth = 0;
        int leftTrim = 0;
        if(pano){
